...
class WorkflowFileTests(unittest.TestCase):
    def test_workflow_has_human_review_and_secret_reference(self):
        ...
        self.assertIn("git diff --cached --name-only --diff-filter=AM", text)
        self.assertIn('body_file="${{ steps.draft_branch.outputs.body_file }}"', text)
        self.assertNotIn("find output/social -name pr-body.md", text)
