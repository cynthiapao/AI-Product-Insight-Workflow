...
from ai_product_insight.sources import FetchError, HttpFetcher


class SmokeFetcher(HttpFetcher):
    def __init__(self, block_listings: bool):
        super().__init__(timeout=15, retries=0)
        self.block_listings = block_listings

    def fetch_page(self, url):
        if self.block_listings and (urlsplit(url).hostname or "").endswith("producthunt.com"):
            raise FetchError("Simulated cloud Product Hunt 403")
...
def main() -> None:
    ...
    parser.add_argument("--simulate-blocked-listings", action="store_true")
    ...
    if args.archive:
        ...
        if args.name:
            candidates = [candidate for candidate in candidates if candidate.name == args.name]
            if not candidates:
                parser.error("Requested name not in archive")
    ...
    for candidate in candidates:
        result = collect_research_evidence(candidate, SmokeFetcher(args.simulate_blocked_listings))
        print(json.dumps({
            "name": candidate.name,
